...
class SendQuoteMailer < ApplicationMailer
  ...
  def create_quote_file(quote, price)
    Prawn::Document.generate("#{Rails.root}/tmp/quote_#{@info.id.to_s}.pdf") do |pdf|
      ...
      if quote.earth_work
        pdf.text "\u2022 Project requires Topography Survey."
      else
      ...
      total_price = 0
      item = 1
      temp_arr = [{:name => "Item #{item.to_s}\nArch Sets", :price => get_price(get_size(quote.size)).to_s}]
      total_price += get_price_integer(get_size(quote.size))
      item += 1
      if quote.earth_work
        temp_arr.append({:name => "Item #{item.to_s}\nTopo Sets", :price => get_price(get_size(quote.size)).to_s})
        total_price += get_price_integer(get_size(quote.size))
        item += 1
      end
  ...
  def get_price(footage)
    "$" + (footage * 4).to_s
  end

  def get_price_integer(footage)
    footage * 4
  end
end
